...
global cxe cxw ce cw fspacee fspacew P Py k Ny egrid sminw smaxw smine smaxe ...
    ...
    INV_AG INV_USE_EIGS INV_CURV INV_EXIT_RATE COMPUTE_AGE INV_AGE_FRAC110 ...
    INV_AGE_MEAN INV_AGE_MASS INV_AGE_COH INV_AGE_KEEP
...
if ~isempty(COMPUTE_AGE) && COMPUTE_AGE
    ...
    tot             = sum(age_mass);
    INV_AGE_FRAC110 = sum(age_mass(1:min(10,AMAX))) / max(tot, 1e-300);
    INV_AGE_MEAN    = sum((1:AMAX)' .* age_mass)    / max(tot, 1e-300);
    INV_AGE_MASS    = age_mass;
    fprintf('  [firm-age] frac aged 1-10 = %.4f   mean age = %.1f   (Sum cohorts=%.5f vs meP=%.5f)\n', ...
        INV_AGE_FRAC110, INV_AGE_MEAN, tot, sum(meP));
end
end

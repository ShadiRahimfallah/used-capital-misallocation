...
global rho alpha eta beta delta W r smin smax fspacee fspacew fspace ce cw ...
    ...
    VF_WARMSTART_XCALL hybrid_use_nested_clearing hybrid_inner_clearing_skip ...
    hybrid_nested_price_lb hybrid_nested_price_ub ...
    ergodic_skip_legacy_moments cf_cf1_zero_used_prices GRID_LEGACY ...
    amax_calib_mult VF_N_OUTER VF_N_PICARD VF_N_NEWTON VF_N_HOWARD DtoY ...
    MOMENTS_FROM_INVARIANT equilibrium_cf_eval DROP_CLEARING_WEIGHTS ...
    firm_exit_target top10_emp_share_target nested_clearing_tol ...
    nested_clearing_tol_Knew calib_moment_weights R_TARGET WGINI_USE_TRIM
...
if ~calib_mode
    fprintf('\n--- EVALUATING x ---\n');
    fprintf('  W=%.4f  r=%.4f  Rnew=%.4f (=r+delta)  theta=%.4f\n', ...
        W, r, Rnew, theta);
    fprintf('  eta_p=%.4f  omega=%.3f  pw=%.3f  sigma_y=%.3f   [single capital: no qu/pu/zeta/kappa]\n', ...
        eta_p, omega, productivity_weight, sigma_y);
end

if r <= -delta
    y                     = 100;
    gaps_calibration_last = 11 * ones(6, 1);
    fprintf('PENALTY: r=%.4f implies Rnew=r+delta=%.4f <= 0\n', r, r + delta);
    return;
end
...
try
    ...
    if ~isempty(VF_N_NEWTON), n_newton = VF_N_NEWTON; end
    ...
    if MOMENTS_FROM_INVARIANT
        ...
        else, gap_top10 = 0; end

        gaps_calibration_last = [gap_Knew; gap_Labor; gap_DtoY; ...
            gap_wageGini;
            gap_exit;
            gap_top10];
        ...
        if equilibrium_cf_eval
            clr_cap = max(-3, min(3, [gap_Knew; gap_Labor]));
            y       = sqrt(clr_cap(1)^2 + clr_cap(2)^2);
            ...
            mkt_obj = sqrt(gKn_mkt^2 + clr_cap(2)^2);

            if isempty(R_TARGET), R_TARGET_use = []; else, R_TARGET_use = R_TARGET; end
            mom_vec = [gap_DtoY; gap_wageGini; gap_exit; gap_top10];
            if ~isempty(R_TARGET_use)
                mom_vec = [mom_vec; (r - R_TARGET_use) / max(R_TARGET_use, 1e-6)];
            end
            if ~isempty(calib_moment_weights) && numel(calib_moment_weights) == numel(mom_vec)
                w_mom = calib_moment_weights(:);
            else
                w_mom = 25 * ones(numel(mom_vec), 1);
            end
        ...
        if ~isempty(calib_mode) && ~calib_mode
            fprintf(['  [TAN-FAST] y=%.6f  Knew=%+.1f%% Lab=%+.1f%% DtoY=%+.1f%% ' ...
                'wG=%+.1f%% exit=%+.1f%% t10=%+.1f%%  ent=%.3f\n'], ...
                y, 100*gap_Knew, 100*gap_Labor, 100*gap_DtoY, ...
                100*gap_wageGini, 100*gap_exit, 100*gap_top10, ent_share_sim);
        end
        return;
    end
    ...
    if exist('L','var') && exist('ent_mask','var') && sum(ent_mask(:)) > 10
        ...
        if exist('L_supply_sim','var') && exist('L_supply_heads','var') && L_supply_heads > 0
            Lbar_worker_sim = L_supply_sim / L_supply_heads;
            firm_size_heads = firm_size_model / max(Lbar_worker_sim, 1e-20);
        else
            Lbar_worker_sim = NaN;
            firm_size_heads = NaN;
        end
    ...
    y_vec = [gap_Knew; gap_Labor; gap_DtoY; ...
        gap_wageGini;
        gap_exit;
        gap_top10];
    ...
    if equilibrium_cf_eval
        ...
        mom_vec = [gap_DtoY; gap_wageGini; gap_exit; gap_top10];
        if ~isempty(R_TARGET)
            mom_vec = [mom_vec; (r - R_TARGET) / max(R_TARGET, 1e-6)];
        end
        if ~isempty(calib_moment_weights) && numel(calib_moment_weights) == numel(mom_vec)
            w_mom = calib_moment_weights(:);
        else
    ...
    if calib_mode
        ...
        fprintf('  PARAM| W=%.4f r=%.4f Rnew=%.4f (=r+d) th=%.3f eta=%.3f etap=%.4f om=%.3f pw=%.3f sgy=%.3f exP=%.4f ent=%.3f\n', ...
            W, r, Rnew, theta, eta, eta_p, omega, productivity_weight, sigma_y, exit_prob, ent_share_sim);
        fprintf('  --- MARKET CLEARING, 2 markets (mkt_obj=%.4f) ---\n', mkt_obj);
        fprintf('  (1) gap_Knew   demand=%.5f  supply=%.5f  gap=%+6.1f%%\n', ...
            Knew_dem_erg, Knew_sup_erg, 100*gap_Knew_erg);
        fprintf('  (2) gap_Labor  demand=%.5f  supply=%.5f  gap=%+6.1f%%\n', ...
            L_dem_erg, L_sup_erg, 100*gap_Labor_erg);
        fprintf('  --- 4 MOMENTS (moment_obj=%.4f) ---\n', moment_obj);
        mom_contrib = mom_cap.^2 .* w_mom;
        fprintf('  (3) gap_DtoY   model=%.4f   target=%.4f  gap=%+6.1f%%  cost=%.4f  [GROSS, matches data]\n', ...
            DtoY_sim, DtoY, 100*gap_DtoY, mom_contrib(1)/sum_w);
        if exist('DtoY_sim_net','var') && ~isnan(DtoY_sim_net)
            fprintf('  (--) DtoY_net  model=%.4f                          (net debt, savers offset)\n', DtoY_sim_net);
        end
        if exist('gini_wage_score','var') && isfinite(gini_wage_score), wg_rep = gini_wage_score; else, wg_rep = gini_wage_model; end
        fprintf('  (4) gap_wGini scored=%.4f (untrim %.4f)  target=%.3f  gap=%+6.1f%%  cost=%.4f\n', ...
            wg_rep, gini_wage_model, wage_gini_target, 100*gap_wageGini, mom_contrib(2)/sum_w);
        fprintf('  (5) gap_exit model=%.4f   target=%.5f gap=%+6.1f%%  cost=%.4f  [WEIGHT 0 -- OUTCOME]\n', ...
            firm_exit_model, firm_exit_target, 100*gap_exit, mom_contrib(3)/sum_w);
        fprintf('  (6) gap_top10 model=%.4f   target=%.5f gap=%+6.1f%%  cost=%.4f\n', ...
            top10_share_model, top10_emp_share_target, 100*gap_top10, mom_contrib(4)/sum_w);
        fprintf('  (--) DIAGNOSTIC: firm size = %.2f EMPLOYEES/firm  (%.3f efficiency units)\n', ...
            firm_size_heads, firm_size_model);
        fprintf('  --- MOMENT WEIGHT TABLE (4 moments; exit weight 0 = outcome) ---\n');
        moment_lbls = {'DtoY','wGini','exit','top10'};
        fprintf('  %-12s %8s %8s %10s\n','moment','w','|gap|%','wcost');
        for ii = 1:4
            fprintf('  %-12s %8d %8.1f %10.4f\n', ...
                moment_lbls{ii}, w_mom(ii), 100*abs(mom_cap(ii)), mom_contrib(ii)/sum_w);
        end
        fprintf('  mkt_obj=%.4f  moment_obj=%.4f  obj=%.4f\n', mkt_obj, moment_obj, y);
    else
        fprintf('\n  === ALL GAPS (hybrid: clearing=ergodic, moments=simulate) ===\n');
        fprintf('  (1) gap_Knew       gap=%7.2f%%  [ergodic]\n', 100*gap_Knew);
        fprintf('  (2) gap_Labor      gap=%7.2f%%  [ergodic]\n', 100*gap_Labor);
        fprintf('  (3) gap_DtoY       model=%9.4f  target=%9.4f  gap=%7.2f%%  [simulate]\n', ...
            DtoY_sim, DtoY, 100*gap_DtoY);
        fprintf('  (--) single capital: Ku=0, Rnew=r+delta=%.4f\n', Rnew);
